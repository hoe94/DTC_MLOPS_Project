...
def load_data():
    ...
    X = df.drop(['ID', 'Customer_ID', 'Month', 'Name', 'SSN', 'Type_of_Loan', 'Credit_History_Age', 'Credit_Score'], axis = 1)
    y = df['Credit_Score']
...
def register_best_model(experiment_id, run_name):
    ...
    if best_performace_runs_name != run_name: #If the best performance runs is not from the current run
        pass
    else:
        model_uri = best_performace_runs.info.run_id
        mlflow.register_model(model_uri = f'runs:/{model_uri}/artifact_folder', name = 'Credit_Score_Model')
